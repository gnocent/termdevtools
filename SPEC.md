...
- **Auto-completion (`Tab` or `F10`, left panel focused)**: offered only when the cursor is in the middle of typing a `METHOD partial_endpoint` line (not in a JSON body or anywhere else — `Tab` keeps its standard tab-insertion behavior there; `F10` has no such fallback meaning and is simply swallowed). `F10` was added after confirming Tab itself gets swallowed before reaching the app on some terminals (see §4); no fallback modifier detection can work around a key that never reaches the app at all. Case-insensitive comparison of the typed prefix against a **list of known endpoints**, centered on administration/operations (`_cat/*`, `_cluster/*`, `_nodes/*`, index admin endpoints...) — no dynamic discovery of real index names (noted as an idea for a future version, see §7).
