...
| Compléter un endpoint en cours de frappe | `Tab` ou `F10` dans pannel gauche, sur une ligne `MÉTHODE endpoint` (§3.2) | Défini |
...
>
> **macOS** : `Ctrl+←/→` est intercepté au niveau du système par défaut (changement d'espace Mission Control). `Option`/`Alt` est accepté comme modificateur supplémentaire, en plus de `Ctrl`, pour changer de panneau (`Ctrl+←/→` / `Option+←/→`) — voir `hasShortcutModifier` dans `ui/app.go`. Volontairement **pas étendu** à `Ctrl+F`/`Ctrl+S`/`Ctrl+C` : ce sont des octets de contrôle bruts, fiables partout, sans ce type de conflit, et par défaut les terminaux macOS transforment `Option`+lettre en caractère accentué plutôt que de signaler un modificateur (sauf activation de « Use Option as Meta key » dans les préférences du terminal).
>
> `Ctrl+Entrée` (exécuter) et `Ctrl+Maj+←/→` (redimensionner) ont chacun fait l'objet d'une investigation dédiée, en supposant d'abord qu'ils se répareraient de la même façon que les flèches ci-dessus (`Option`/`Alt` en modificateur supplémentaire). Un petit outil de diagnostic ([`cmd/keydebug`](cmd/keydebug/main.go)) construit pour vérifier cela a plutôt trouvé, sur un vrai terminal macOS :
> - pour Entrée : **aucun modificateur n'atteint l'appli du tout** — `Ctrl+Entrée`, `Option+Entrée` et `Alt+Entrée` sont tous rapportés comme un simple `Entrée` (même touche, mêmes modificateurs à zéro). Pas réparable en changeant de modificateur : `Ctrl+M` *est* le code de contrôle d'`Entrée`, et ce terminal n'y attache aucun encodage étendu, quel que soit le modificateur.
> - pour `Option`/`Alt+←/→` spécifiquement : rapporté non pas comme une flèche modifiée, mais comme l'encodage classique de navigation par mot Meta-b/Meta-f (`Echap b` / `Echap f`) — un simple caractère `'b'`/`'f'` avec seulement le modificateur Alt, et une valeur de `Key` totalement différente de `KeyLeft`/`KeyRight`. Géré en reconnaissant en plus cet encodage (`isAltWordBack`/`isAltWordForward` dans `ui/app.go`) — sans quoi le repli `Option`/`Alt` ci-dessus ne se déclenche jamais pour les flèches sur ce terminal, l'événement ne ressemblant même pas à une flèche au départ.
> - pour `Maj+Alt+←/→` (tentative d'atteindre le redimensionnement via `Option`) : rapporté comme un `KeyLeft`/`KeyRight` **simple, non modifié** — indiscernable, au niveau de l'événement clavier, d'une simple pression de flèche sans modificateur. Aucun encodage n'existe sur ce terminal pour transmettre cette combinaison.
>
> Vu ces deux derniers constats, le redimensionnement ne peut pas être atteint de façon fiable via une combinaison basée sur Entrée ou les flèches sur tous les terminaux. Le correctif a été le même pour les deux raccourcis : ne plus du tout dépendre de la touche ambiguë. `Ctrl+E` (exécuter) et `F5`/`F6` (redimensionner) sont désormais les déclencheurs principaux, garantis fiables — un octet de contrôle brut et des touches de fonction, sans aucune ambiguïté d'encodage de modificateur. `Ctrl+Entrée` et `Ctrl+Maj+←/→` (`Option`/`Alt` inclus) ne sont conservés qu'en complément best-effort, pour les terminaux qui les rapportent.
>
> **`Tab` absorbé entièrement sur certains terminaux** : contrairement à tous les cas ci-dessus — toujours une question d'encodage de modificateur *ambigu* ou *manquant*, la touche de base elle-même atteignant toujours l'appli — un simple `Tab` a été rapporté, sur une machine réelle, comme n'atteignant **pas du tout** l'appli sur deux terminaux pourtant sans rapport (`cmd.exe` Windows et PuTTY, tous deux sur cette même machine) : aucun événement clavier du tout, confirmé via `cmd/keydebug`. `Tab` est par ailleurs une touche aussi simple et universellement fiable que `Ctrl+E`/`Ctrl+F`/`Ctrl+S`/`Ctrl+C` (tous confirmés fiables sur ces mêmes machines) — ce n'est pas une bizarrerie d'encodage décodable qu'une logique à la `hasShortcutModifier` pourrait contourner, quelque chose en amont de l'appli l'absorbe purement et simplement, très probablement pour ses propres besoins de navigation entre éléments. `F10` a été ajouté comme alternative garantie fiable (`isCompletionShortcut` dans `ui/app.go`) — `Tab` reste le déclencheur principal, attendu par défaut.
