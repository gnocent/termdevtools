...
>
> **macOS** : `Ctrl+←/→` est intercepté au niveau du système par défaut (changement d'espace Mission Control), et `Ctrl+Entrée` ne peut même pas être distingué d'un simple `Entrée` dans l'encodage terminal classique — `Ctrl+M` *est* le code de contrôle d'`Entrée`. `Option`/`Alt` est donc accepté comme modificateur supplémentaire, en plus de `Ctrl`, pour exactement les trois raccourcis concernés : exécuter (`Ctrl+Entrée` / `Option+Entrée`), changer de panneau (`Ctrl+←/→` / `Option+←/→`) et redimensionner (`Ctrl+Maj+←/→` / `Option+Maj+←/→`) — voir `hasShortcutModifier` dans `ui/app.go`. Volontairement **pas étendu** à `Ctrl+F`/`Ctrl+S`/`Ctrl+C` : ce sont des octets de contrôle bruts, fiables partout, sans ce type de conflit, et par défaut les terminaux macOS transforment `Option`+lettre en caractère accentué plutôt que de signaler un modificateur (sauf activation de « Use Option as Meta key » dans les préférences du terminal).
